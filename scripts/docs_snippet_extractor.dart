...
/// Returns import lines from [code] that are safe to use in a test file.
///
/// Strips imports for [dart:io], [dart:html], and [dart:isolate] (incompatible
/// with the VM test runner without platform guards) and returns all others.
/// Callers should deduplicate the results before adding them to a file header.
List<String> extractCompatibleImports(String code) {
  const incompatible = {'dart:io', 'dart:html', 'dart:isolate'};
  return code
      .split('\n')
      .where((line) {
        final trimmed = line.trimLeft();
        if (!trimmed.startsWith('import ')) return false;
        return !incompatible.any((lib) => trimmed.contains("'$lib'") || trimmed.contains('"$lib"'));
      })
      .toList();
}
...
String generateFileHeader({List<String> extraImports = const []}) {
  // Base imports always present in generated test files.
  final baseImports = [
    "import 'dart:convert';",
  ...
  // Add any extra imports from snippets that aren't already in the base set.
  final allImports = [...baseImports];
  for (final imp in extraImports) {
    if (!allImports.contains(imp.trim())) {
      allImports.add(imp.trim());
    }
  }

  final importBlock = allImports.join('\n');
...
String generateTestFile(List<SnippetGroup> groups) {
  ...
  // Collect compatible imports from all snippets across all groups so that
  // doc examples using non-barrel packages (e.g. dart:math) don't silently
  // lose their imports when stripImports removes them from the snippet body.
  final snippetImports = <String>[];
  for (final group in groups) {
    for (final snippet in group.snippets) {
      snippetImports.addAll(extractCompatibleImports(snippet.code));
    }
  }

  buffer.writeln(generateFileHeader(extraImports: snippetImports));
