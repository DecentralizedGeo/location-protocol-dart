...
- **Script secret fallback**: For ad hoc scripts, prefer `.env`/`Platform.environment` first, then generate an ephemeral in-memory key only when the private key is missing. Never persist the fallback key to disk.
